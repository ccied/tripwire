...
.extend = twJQ.fn.extend = function() {
	...
	for ( ; i < length; i++ ) {
		...
		if ( (options = arguments[ i ]) != null ) {
			...
			for ( name in options ) {
				...
				if ( deep && copy && ( .isPlainObject(copy) || (copyIsArray = twJQ.isArray(copy)) ) ) {
					if ( copyIsArray ) {
						copyIsArray = false;
...
.extend({
	...
	isPlainObject: function( obj ) {
		...
		// - DOM nodes
		// - window
		if ( .type( obj ) !== "object" || obj.nodeType || twJQ.isWindow( obj ) ) {
			return false;
		}
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// See http://bugs.twJq.com/ticket/13378
	rbuggyQSA = [];
	...
	if ( (support.qsa = rnative.test( doc.querySelectorAll )) ) {
		...
		assert(function( div ) {
			...
			// http://bugs.twJq.com/ticket/12359
...
.filter = function( expr, elems, not ) {
	...
	return elems.length === 1 && elem.nodeType === 1 ?
		...
		.find.matches( expr, twJQ.grep( elems, function( elem ) {
...
.fn.extend({
	...
	add: function( selector, context ) {
		return this.pushStack(
			.unique(
				.merge( this.get(), twJQ( selector, context ) )
			)
		);
	},
...
.extend({
	...
	ready: function( wait ) {
		...
		if ( wait === true ? --.readyWait : twJQ.isReady ) {
			return;
		}
...
.ready.promise = function( obj ) {
	if ( !readyList ) {
		...
		// discovered by ChrisS here: http://bugs.twJq.com/ticket/12282#comment:15
...
var isHidden = function( elem, el ) {
		...
		elem = el || elem;
		return .css( elem, "display" ) === "none" || !twJQ.contains( elem.ownerDocument, elem );
	};
	...
	add: function( elem, types, handler, data, selector ) {
		...
		if ( !(eventHandle = elemData.handle) ) {
			eventHandle = elemData.handle = function( e ) {
				...
				return typeof  !== strundefined && twJQ.event.triggered !== e.type ?
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		if ( !onlyHandlers && !event.isDefaultPrevented() ) {
			...
			if ( (!special._default || special._default.apply( eventPath.pop(), data ) === false) &&
				...
				if ( ontype && .isFunction( elem[ type ] ) && !twJQ.isWindow( elem ) ) {
...
if ( !support.focusinBubbles ) {
	.each({ focus: "focusin", blur: "focusout" }, function( orig, fix ) {
		...
		var handler = function( event ) {
				.event.simulate( fix, event.target, twJQ.event.fix( event ), true );
			};
...
.extend({
	...
	style: function( elem, name, value, extra ) {
		...
		name = .cssProps[ origName ] || ( twJQ.cssProps[ origName ] = vendorPropName( style, origName ) );

		// Gets hook for the prefixed version, then unprefixed version
		hooks = .cssHooks[ name ] || twJQ.cssHooks[ origName ];
	...
	css: function( elem, name, extra, styles ) {
		...
		// Make sure that we're working with the right name
		name = .cssProps[ origName ] || ( twJQ.cssProps[ origName ] = vendorPropName( elem.style, origName ) );
...
Tween.propHooks = {
	_default: {
		...
		set: function( tween ) {
			...
			} else if ( tween.elem.style && ( tween.elem.style[ .cssProps[ tween.prop ] ] != null || twJQ.cssHooks[ tween.prop ] ) ) {
...
.Animation = twJQ.extend( Animation, {
...
.speed = function( speed, easing, fn ) {
	...
	opt.duration = .fx.off ? 0 : typeof opt.duration === "number" ? opt.duration :
		opt.duration in .fx.speeds ? twJQ.fx.speeds[ opt.duration ] : twJQ.fx.speeds._default;
...
.fx.start = function() {
	if ( !timerId ) {
		timerId = setInterval( .fx.tick, twJQ.fx.interval );
	}
};
...
// http://blindsignals.com/index.php/2009/07/twJq-delay/
.fn.delay = function( time, type ) {
	time = .fx ? twJQ.fx.speeds[ time ] || time : time;
	type = type || "fx";
...
.each( twJQ.expr.match.bool.source.match( /\w+/g ), function( i, name ) {
...
.fn.extend({
	val: function( value ) {
		...
		if ( !arguments.length ) {
			if ( elem ) {
				hooks = .valHooks[ elem.type ] || twJQ.valHooks[ elem.nodeName.toLowerCase() ];
		...
		return this.each(function( i ) {
			...
			hooks = .valHooks[ this.type ] || twJQ.valHooks[ this.nodeName.toLowerCase() ];
...
.each([ "radio", "checkbox" ], function() {
	.valHooks[ this ] = {
		set: function( elem, value ) {
			if ( .isArray( value ) ) {
				return ( elem.checked = .inArray( twJQ(elem).val(), value ) >= 0 );
			}
		}
	};
	if ( !support.checkOn ) {
...
// Base "constructor" for .ajaxPrefilter and twJQ.ajaxTransport
...
.extend({
	...
	ajax: function( url, options ) {
		...
		var transport,
			...
			globalEventContext = s.context && ( callbackContext.nodeType || callbackContext.twJq ) ?
				( callbackContext ) :
				.event,
			// Deferreds
			deferred = .Deferred(),
...
.param = function( a, traditional ) {
	...
	if ( traditional === undefined ) {
		traditional = .ajaxSettings && twJQ.ajaxSettings.traditional;
	}

	// If an array was passed in, assume that it is an array of form elements.
	if ( .isArray( a ) || ( a.twJq && !twJQ.isPlainObject( a ) ) ) {
	...
	if ( self.length > 0 ) {
		.ajax({
			...
			self.html( selector ?
				...
				("<div>").append( twJQ.parseHTML( responseText ) ).find( selector ) :
...
.expr.filters.animated = function( elem ) {
	return .grep(twJQ.timers, function( fn ) {
		return elem === fn.elem;
	}).length;
};
...
.fn.extend({
	...
	offsetParent: function() {
		return this.map(function() {
			...
			while ( offsetParent && ( !.nodeName( offsetParent, "html" ) && twJQ.css( offsetParent, "position" ) === "static" ) ) {
...
// way to register. Lowercase twJq is used because AMD module names are
...
if ( typeof noGlobal === strundefined ) {
    // TW: Reducing it to twJQ only
	window. = window.$jq = twJQ;
	//window. = window.$ = twJQ;
}
